...
async def async_setup_entry(
    ...
    async_add_entities(
        [
            ...
            PersonalizedMovieFeedSensor(coordinator, entry),
            TVDiscoveryFeedSensor(coordinator, entry),
            PersonalizedTVFeedSensor(coordinator, entry),
            OscarsFeedSensor(coordinator, entry),
...
def _movie_feed_item(item: dict[str, Any], source: str) -> dict[str, Any]:
    providers = (
        item.get("my_providers")
        or item.get("providers")
        or []
...
def _tv_feed_item(item: dict[str, Any], source: str) -> dict[str, Any]:
    providers = (
        item.get("my_providers")
        or item.get("providers")
        or []
...
class PersonalizedMovieFeedSensor(_MediaTrackerFeedSensor):
    _attr_name = "Personalized movies"
    _attr_icon = "mdi:movie-filter"

    def __init__(self, coordinator, entry) -> None:
        super().__init__(coordinator, entry)
        self._attr_unique_id = f"{entry.entry_id}_personalized_movies"

    @property
    def _items(self) -> list[dict[str, Any]]:
        return [
            _movie_feed_item(item, "personalized")
            for item in self.coordinator.data.get("personalized_movies", [])
        ]


class TVDiscoveryFeedSensor(_MediaTrackerFeedSensor):
    _attr_name = "TV discovery"
    _attr_icon = "mdi:television-guide"

    def __init__(self, coordinator, entry) -> None:
        super().__init__(coordinator, entry)
        self._attr_unique_id = f"{entry.entry_id}_discovery_tv"

    @property
    def _items(self) -> list[dict[str, Any]]:
        return [
            _tv_feed_item(item, "discovery")
            for item in self.coordinator.data.get("discovered_tv", [])
        ]


class PersonalizedTVFeedSensor(_MediaTrackerFeedSensor):
    _attr_name = "Personalized TV"
    _attr_icon = "mdi:television-classic"

    def __init__(self, coordinator, entry) -> None:
        super().__init__(coordinator, entry)
        self._attr_unique_id = f"{entry.entry_id}_personalized_tv"

    @property
    def _items(self) -> list[dict[str, Any]]:
        return [
            _tv_feed_item(item, "personalized")
            for item in self.coordinator.data.get("personalized_tv", [])
        ]


class OscarsFeedSensor(_MediaTrackerFeedSensor):
    """Latest Best Picture Oscar nominees and winner."""
    ...
    def __init__(self, coordinator, entry) -> None:
        super().__init__(coordinator, entry)
        self._attr_unique_id = f"{entry.entry_id}_oscars"

    @property
    def _items(self) -> list[dict[str, Any]]:
        return [
            self._movie_item(item)
            for item in self.coordinator.data.get("oscar_movies", [])
        ]
